...
fun question_screen(quizs: List<Quiz>) {
    ...
    for(i in quizs.indices){
        ...
        Log.e("HiGanesh", question.toString() )
        questions.add(question)

                }


    Log.e("HiGanesh", questions.indices.toString() )
    for(i in questions.indices){
        Log.e("HiGanesh", questions[i].toString() )
    }
    ...
    // Start a countdown timer when the question screen is shown
    LaunchedEffect(key1 = currentQuestion) {
        ...
        // If the timer has ended and the user hasn't answered, show the correct answer
        if (timer == 0) {
            ...
            if (currentQuestionIndex < questions.size - 1) {
                ...
                // Reset the background colors and timer for the next question
